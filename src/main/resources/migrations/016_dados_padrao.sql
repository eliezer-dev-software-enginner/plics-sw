INSERT INTO preferencias (tema, credenciais_habilitadas, primeiro_acesso, data_criacao, login, senha)
...
SELECT 'Geral', strftime('%s', 'now') * 1000
WHERE NOT EXISTS (SELECT 1 FROM categoria WHERE nome = 'Geral');

INSERT INTO fornecedores (nome, data_criacao)
SELECT 'Fornecedor Padrão', strftime('%s', 'now') * 1000
WHERE NOT EXISTS (SELECT 1 FROM fornecedores WHERE nome = 'Fornecedor Padrão');

INSERT INTO usuarios (nome, senha, cargo, data_criacao)
SELECT 'admin', '1234', 'admin', strftime('%s', 'now') * 1000
WHERE NOT EXISTS (SELECT 1 FROM usuarios WHERE nome = 'admin');

INSERT INTO empresas (texto_responsabilidade, data_criacao)
...
INSERT INTO clientes (nome, cpf_cnpj, celular, email, data_criacao)
