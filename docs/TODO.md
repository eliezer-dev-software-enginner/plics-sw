...
## Concluído (Select de Cliente da venda — default "CLIENTE PADRÃO" no add + fix do toString em edit/clone — 2026-09-20 tarde)
- [x] **Bug 1 — venda nova abria com Select de Cliente vazio** (regressão do commit `3614291`): `ScreenAddOrEditVenda` chamava `fetchListData(false)` fixo, mas a pré-seleção do "CLIENTE PADRÃO" (id == 1L) só roda com o flag `true`. Fix: ctor passou a chamar `fetchListData("add".equals(type))` — pré-seleciona só em inclusão.
- [x] **Bug 2 — edit/clone exibia `ClienteModel@...` (toString)**: (a) `SelectColumn` com `compareById` aplicava `.value()` ANTES de `displayText`/comparador — o subscriber imediato rodava com a célula padrão `toString` e o cliente já populado; (b) `Select.compareById()` do framework usa `getDeclaredField("id")`, que NÃO enxerga o `id` herdado de `Identifier` → nunca casava com o item da lista. Fix no app: `displayText` + comparador `Components::sameId` (via `Identifier.getId()`, nulo-safe) aplicados ANTES do `.value()` nos 3 overloads de `SelectColumn`. Framework intocado (decisão do usuário).
