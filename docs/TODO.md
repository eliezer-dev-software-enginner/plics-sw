...
- [ ] **PedidosScreen não tem campo de busca** — o teste antigo #110 ("Filtrar pedidos por busca") descrevia uma funcionalidade inexistente (removido do `.md`). Além disso a coluna "Cliente" da tabela mostra o `clienteId` numérico, não o nome, e `matchesSearch()` busca por `clienteId`/`formaPagamento`. Decidir se vale adicionar `Components.searchInput` + exibir nome do cliente
- [ ] **Avaliar eliminar o updater no build para a Microsoft Store**: o pacote com updater embutido (`scripts/create-msi-with-updater.py`, `my_app.updater.Main`/`HomeScreenViewModel`) baixa e reinstala o próprio `.msi` — modelo incompatível com o de atualização automática da Microsoft Store, e o `.exe` extra do updater dentro do pacote pode confundir revisão/certificação (mesmo já com mensagem amigável ao ser aberto sem argumentos, 2026-07-28). Se o build for especificamente para publicação na Store, considerar gerar com `scripts/create-msi.py` (sem `--add-launcher`/updater) em vez de `create-msi-with-updater.py`, deixando a atualização por conta do próprio mecanismo da Store — mesmo padrão já adotado pro Flatpak (`Main.isFlatpak`, ver decisão de 2026-07-24)
