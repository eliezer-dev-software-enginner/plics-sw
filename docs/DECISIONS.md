...
**Decisão:**
1. `AuthScreenViewModel.entrar()`: substituir `day > 11` por `isLicensaTesteExpirada(licenseValue)` — usa a mesma lógica do `load()`.
...
2. Salvar a licença utilizada no campo `license` da tabela `preferencias` (Migration V17).
