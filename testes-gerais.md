...
| 203 | Devolver venda à vista do histórico (ex: caso #84 do PDVScreen) | Alerta de confirmação. Ao confirmar: venda permanece na lista (não é excluída) com coluna "Status" = "Devolvida"; estoque do(s) produto(s) vendido(s) é devolvido (conferir em ProdutoScreen); os cards "Receitas", "Lucro líquido" e "Hoje você fez" na Home são recalculados refletindo a devolução. Diferente de "Excluir", o histórico da venda não desaparece. | | |
| 204 | Devolver venda fiada do histórico (ex: caso #85 do PDVScreen) | Alerta de confirmação menciona que o pagamento vinculado será estornado. Ao confirmar: estoque devolvido; as parcelas correspondentes em ContasAReceberScreen são canceladas e removidas; pedido permanece na lista com coluna "Status" = "Devolvida" (diferente de excluir). | | |
| 205 | Tentar devolver venda já devolvida | Botão "Devolver venda selecionada" não aparece mais quando a venda selecionada já está com Status "Devolvida" (só aparece para vendas ainda não devolvidas). | | |
| 208 | Trocar venda à vista do histórico (ex: caso #84 do PDVScreen, trocando por outro produto) | Botão "Trocar venda selecionada" abre modal com busca de produto. Ao adicionar itens e confirmar: venda original fica com Status "Devolvida"; um NOVO pedido aparece no histórico (ID novo) com os produtos escolhidos e o total deles; estoque do produto original volta e o do novo é baixado (conferir em ProdutoScreen); Home recalculada. | | |
| 209 | Trocar venda fiada do histórico | As parcelas da venda original são canceladas/removidas de ContasAReceberScreen; o pedido novo NUNCA é fiado (não gera parcelas novas — a diferença de valor é tratada presencialmente). Forma de pagamento do modal já vem preenchida com a da venda original, mas pode ser alterada. | | |
| 210 | Troca com produto que não aceita devolução/troca ("Não" no cadastro) | Alerta de erro: "Devolução bloqueada — não aceita(m) devolução/troca: ...". Nada muda: estoque intacto, venda não marcada como devolvida, nenhum pedido novo criado. | | |
| 211 | Confirmar troca sem adicionar produto / editar quantidade acima do estoque | Sem item adicionado, confirmar mostra "Adicione pelo menos um produto para a troca.". Quantidade acima do estoque disponível mostra alerta de estoque insuficiente (considerando o que já está na lista da troca). | | |
